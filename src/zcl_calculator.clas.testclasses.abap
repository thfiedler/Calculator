class ltcl_calculator_test definition final for testing
  ...
  private section.
    methods:
      ...
      test_multiply_2_by_2 for testing raising cx_static_check,
      test_devide_8_by_2 for testing raising cx_static_check,
    test_sqrt_from_16 for testing raising cx_static_check.
endclass.
...
class ltcl_calculator_test implementation.
  ...
  method test_sqrt_from_16.
    data(calc) = new zcl_calculator(  ).
    data(result) = calc->square_root( 16 ).

    cl_abap_unit_assert=>assert_equals(
      exporting
        act                  = result
        exp                  = 4
        msg                  = 'SQRT not working'
    ).
  endmethod.
